...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            long[] array = Console.ReadLine().Split(' ').Select(i => long.Parse(i)).ToArray();
            int[] ans = Enumerable.Repeat(0, n).ToArray();
            for(int i = 0;i < n;i++) {
                ans[i] = array.Where(x => x > array[i]).Distinct().Count();
            }

            for(int i = 0;i < n;i++) {
                Console.WriteLine(ans.Where(e => e == i).Count());
            }
...
namespace x
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] ans = Enumerable.Repeat(0, n).ToArray();
            for(int i = 0;i < n;i++) {
                // long[] tmp = array.Where(i => i > array[i]).Distinct().Count();
                ans[i] = array.Where(i => i > array[i]).Distinct().Count();
            }

            for(int i = 0;i < n;i++) {
                Console.WriteLine(Array.FindAll(ans, e => e == i).Length);
            }
